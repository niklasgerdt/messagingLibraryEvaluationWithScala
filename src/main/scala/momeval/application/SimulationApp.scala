...
object SimulationApp extends Logging {
  ...
  def apply(eventlen: String, pauselen: String, publisher: String) = {
    ...
    def storer() = (e: Event) => {} //drop published events
    ...
    List(
      ...
      new Simulator(supplier(1), pub("tcp://localhost:5001"), pauser, storer(), kill),
      new Simulator(supplier(2), pub("tcp://localhost:5002"), pauser, storer(), kill),
      new Simulator(supplier(3), pub("tcp://localhost:5003"), pauser, storer(), kill),
      new Simulator(supplier(4), pub("tcp://localhost:5004"), pauser, storer(), kill))
      .par
      .foreach(_.simulate())
  }
}
